...
export const PlaygroundToolsPopover = () => {
  ...
  return (
    <Command className="flex flex-col">
      <CommandInput
        placeholder={t("playground.tools.search")}
        className="h-8 border-none py-1 pr-1 pl-6 focus:ring-0 focus:ring-offset-0"
      />
      <CommandList className="max-h-[300px] overflow-y-auto">
        <CommandEmpty>{t("playground.tools.notFound")}</CommandEmpty>
        <CommandGroup>
          {savedTools.map((tool) => (
            <CommandItem
              key={tool.id}
...
export const PlaygroundTools = () => {
  ...
  return (
    <ScrollArea className="h-full [&>[data-radix-scroll-area-viewport]]:max-h-[min(45vh,18rem)]">
      {tools.length === 0 ? (
