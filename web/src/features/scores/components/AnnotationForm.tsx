...
function InnerAnnotationForm<Target extends ScoreTarget>({
  ...
  // LFE-7628 閳?root of this form, used to scope the global keydown listener so
  ...
  // LFE-7628 閳?keyboard-first scoring. Real DOM focus is the single source of
  // truth: `閳叢/`閳彵 (and `Tab`) move focus between fields, `1`-`9` pick an option
  // on the *focused* row, and the focused row is highlighted via `:focus-within`.
  // There is deliberately no parallel "active row" state 閳?it previously fought
  ...
  // LFE-7628 閳?keyboard-first scoring, driven by real DOM focus with a
  // spreadsheet-style navigate-vs-edit split (single source of truth, one
  // outline, never trapped):
  //  - `閳叢 / `閳彵 move focus between *rows* (the row container, never into a text
  ...
  //    type, the combobox to open, a toggle to use 閳?閳?.
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // NOT for a drawer this form is mounted inside (the Annotate drawer) 閳?that
      ...
      // and returns to row navigation 閳?there is no newline to insert, so this is
      ...
      if (
        ...
        // isn't committed-and-dropped (mirrors the 閳?Ctrl+Enter complete gate).
        if (
          target.type === "number" &&
        ...
        // Focusing the row blurs the input (its onBlur saves) and resumes 閳?閳?
        if (row) row.focus();
        else target.blur();
        return;
      }
      ...
      // / Skip / Back / "?" page buttons) must NOT drive the form 閳?otherwise
      // 閳?閳?would hijack focus off that button into the score rows.
      ...
      // `閳叢 / `閳彵 move focus between rows (to the row container itself, never
      // into a text field 閳?so navigation is never trapped).
      if (event.key === "ArrowDown" || event.key === "ArrowUp") {
        if (rowEls.length < 2) return;
        // Navigate from a focused row, or enter from the body 閳?but NOT from an
      ...
      // container itself 閳?once a control is focused, Enter is left to it).
      if (event.key === "Enter") {
        if (currentRow && active === currentRow) {
          // First *enabled* control 閳?skip the disabled stale-category chip that
      ...
      if (/^[1-9]$/.test(event.key)) {
        ...
        // Only from the row container itself or its value control 閳?never the
  ...
  // LFE-7628 閳?`Esc` leaves a focused score field (back to its row) WITHOUT
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-[11fr_1fr] items-center py-1">
                          {/* Limit keyboard scoring to the value control, not row actions. */}
